...
public class Fence : Building
{
  ...
  private void Determine_Sprite()
  {
    ...
    if(neighbours["Top-Mid"] != null && Itemmanager.GetComponent<itemmanager>().items_on_grid[(int)grid_pos.x,(int)grid_pos.y + 1]?.name == "Fence")
      sprite_name += "U";

    if(neighbours["Mid-Right"] != null && Itemmanager.GetComponent<itemmanager>().items_on_grid[(int)grid_pos.x + 1,(int)grid_pos.y]?.name == "Fence")
     sprite_name += "R";

    if(neighbours["Bot-Mid"] != null && Itemmanager.GetComponent<itemmanager>().items_on_grid[(int)grid_pos.x,(int)grid_pos.y - 1]?.name == "Fence")
     sprite_name += "D";

    if(neighbours["Mid-Left"] != null && Itemmanager.GetComponent<itemmanager>().items_on_grid[(int)grid_pos.x - 1,(int)grid_pos.y]?.name == "Fence")
     sprite_name += "L";
